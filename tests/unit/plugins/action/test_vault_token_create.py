...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_token_create import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_token_create"
    task.args = {
...
class TestActionModule:
    """Tests for vault_token_create action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/auth/token/create" in action.OPERATION.path
