...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_database_creds import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_database_creds"
    task.args = {
...
class TestActionModule:
    """Tests for vault_database_creds action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/{mount_path}/creds/{role_name}" in action.OPERATION.path
