...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_database_static_role_creds import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_database_static_role_creds"
    task.args = {
...
class TestActionModule:
    """Tests for vault_database_static_role_creds action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            assert action.OPERATION.method == "GET"
            assert "v1/{mount_path}/static-creds/{name}" in action.OPERATION.path
