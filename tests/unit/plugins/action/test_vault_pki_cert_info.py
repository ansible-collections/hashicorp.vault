...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_pki_cert_info import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_pki_cert_info"
    task.args = {
...
class TestActionModule:
    """Tests for vault_pki_cert_info action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/{mount_path}/cert/{serial}" in action.OPERATION.path
