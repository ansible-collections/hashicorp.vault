...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_auth_method_info import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_auth_method_info"
    task.args = {
...
class TestActionModule:
    """Tests for vault_auth_method_info action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/sys/auth" in action.OPERATION.path
