...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_pki_sign import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_pki_sign"
    task.args = {
...
class TestActionModule:
    """Tests for vault_pki_sign action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/{mount_path}/sign/{role}" in action.OPERATION.path
