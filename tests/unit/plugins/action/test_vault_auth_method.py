...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_auth_method import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_auth_method"
    task.args = {
...
class TestActionModule:
    """Tests for vault_auth_method action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            ...
            assert "v1/sys/auth/{path}" in action.OPERATION.path
