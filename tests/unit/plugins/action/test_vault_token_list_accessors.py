...
def action_module():
    ...
    from ansible_collections.hashicorp.vault.plugins.action.vault_token_list_accessors import ActionModule

    task = MagicMock()
    task.action = "hashicorp.vault.vault_token_list_accessors"
    task.args = {
...
class TestActionModule:
    """Tests for vault_token_list_accessors action plugin."""
    ...
    def test_operation_defined(self, action_module):
        ...
        if hasattr(action, "OPERATION") and action.OPERATION is not None:
            assert action.OPERATION.method == "LIST"
            assert "v1/auth/token/accessors/" in action.OPERATION.path
