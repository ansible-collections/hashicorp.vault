...
from vault.plugins.module_utils.vault_exceptions import VaultConnectionError
from vault.plugins.module_utils.vault_exceptions import VaultApiError
from vault.plugins.module_utils.vault_exceptions import VaultPermissionError
from vault.plugins.module_utils.vault_exceptions import VaultSecretNotFoundError
...
def mock_success_response():
    return {
        "data": {
            ...
            "metadata": {
                "created_time": "2023-10-27T12:00:00Z",
                "version": 3,
                "destroyed": False
            }
        }
...
def test_read_secret_latest_version_success(mocker, vault_client, vault_config, mock_success_response):
    ...
    expected_url = f"{vault_config['addr']}/v1/{vault_config['mount_path']}/data/{vault_config['secret_path']}"
    mock_request.assert_called_once_with('GET', expected_url, params={})
    assert secret == mock_success_response['data']

def test_read_secret_specific_version_success(mocker, vault_client, vault_config, mock_success_response):
    mock_request = mocker.patch('requests.Session.request', return_value=MagicMock())
    mock_request.return_value.json.return_value = mock_success_response
    secret_version = 2
    
    secret = vault_client.read_secret(vault_config["mount_path"], vault_config["secret_path"], version=secret_version)
...
def test_connection_error(mocker, vault_client, vault_config):
    mocker.patch('requests.Session.request', side_effect=requests.exceptions.ConnectionError("Failed to connect"))
