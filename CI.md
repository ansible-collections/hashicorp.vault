...
| [tests.yml](.github/workflows/tests.yml) | unit | Executes unit tests via ansible-test and tox-ansible | Per ansible-core support matrix | All supported ansible-core versions (2.16+) |
...
Note: `ansible-test units` uses pytest as its test runner, so the collection's pytest-based unit tests are fully compatible and supported.
...
**Notes:**
- The `sanity` and `unit` workflows use reusable workflows from [`ansible/ansible-content-actions`](https://github.com/ansible/ansible-content-actions) which:
  ...
  - `ansible-test units` uses pytest as its test runner, so pytest-based tests are fully supported
