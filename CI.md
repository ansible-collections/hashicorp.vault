...
The test matrix is automatically generated by `tox-ansible` based on:
...
- The `sanity` and `unit-galaxy` workflows use `tox-ansible` to auto-generate test matrices based on `requires_ansible` and the ansible-core Python support matrix
- Version combinations follow ansible-core's official Python support matrix
- The `tox-ansible.ini` skip list excludes incompatible combinations (e.g., Python 3.12 with devel/milestone which requires Python 3.13+)
