...
def ensure_present(module: AnsibleModule, vault_token: VaultTokens) -> tuple[bool, dict]:
    """
    ...
    if existing:
        if not renew:
            module.exit_json(changed=False, msg="The token is already present; no action required.")
        # Renew token
...
def ensure_absent(module: AnsibleModule, vault_token: VaultTokens) -> tuple[bool, dict]:
    """
    revoke the token
